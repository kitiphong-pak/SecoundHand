...
describe("อ่านเวลาไทยจากข้อความแชท", () => {
  ...
  // คนไม่ได้พูดเป็นตัวเลขเสมอ ("เจอกันบ่ายได้มั้ย") — เดาเวลากลางๆ ของช่วงให้ แต่ต้องติดธงว่าเดา
  describe("ช่วงเวลากว้างๆ ที่ไม่มีตัวเลข", () => {
    it("เดาเวลากลางๆ ของช่วงนั้นให้ พร้อมบอกว่าเป็นการเดา", () => {
      for (const [text, hour] of [
        ["เจอกันบ่ายได้มั้ย", 14],
        ["เปลี่ยนเป็นตอนบ่ายได้มั้ย", 14],
        ["เช้าๆ เลยดีกว่า", 9],
        ["ตอนเย็นสะดวกกว่า", 17],
        ["ค่ำๆ ได้มั้ย", 19],
        ["ดึกไปมั้ย", 22],
        ...
        const p = parseThaiTime(text)!;
        expect(p.hour, text).toBe(hour);
        expect(p.approximate, text).toBe(true);
      }
    });

    // ถ้าลำดับผิด "บ่าย 3 โมง" จะได้ 14:00 ที่เดาจากคำว่า "บ่าย" แทนที่จะเป็น 15:00 ที่ผู้ใช้บอกเอง
    it("ถ้ามีตัวเลขระบุมาด้วย ต้องใช้ตัวเลขนั้น ไม่ใช่ค่าที่เดาจากคำว่าช่วง", () => {
      const p = parseThaiTime("บ่าย 3 โมง")!;
      expect(p.hour).toBe(15);
      expect(p.approximate).toBe(false);
    });

    // ในเว็บขายของมือสอง คำบอกช่วงเวลาชนกับชื่อสินค้าได้ง่ายมาก และคนพิมพ์ชื่อสินค้าบ่อยกว่านัดเวลา
    it("ชื่อสินค้าที่มีคำเหล่านี้ปนอยู่ ต้องไม่กลายเป็นเวลานัด", () => {
      expect(parseThaiTime("ตู้เย็นยังใช้ได้ปกติมั้ยครับ")).toBeNull();
      expect(parseThaiTime("มีสายชาร์จให้ด้วยมั้ย")).toBeNull();
      expect(parseThaiTime("ขอดูสายไฟหน่อย")).toBeNull();
    });

    it("เวลาที่ระบุชัดเจนต้องไม่ถูกติดธงว่าเดา", () => {
      expect(parseThaiTime("ตอน 5 โมง")!.approximate).toBe(false);
      expect(parseThaiTime("17:30")!.approximate).toBe(false);
    });
  });
